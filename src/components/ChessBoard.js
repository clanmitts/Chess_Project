...
const ChessBoard = () => {
    ...
    const highLightGrid = (grid, color) => {
        ...
        contextRef.current.strokeRect(x, y, PIECEWIDTH, PIECEHEIGHT);
    };
    ...
    const drawPiece = (piece) => {
        ...
        if (!piece.selected) {
            contextRef.current.drawImage(image, figureX, figureY, PIECEWIDTH, PIECEHEIGHT, x, y, PIECEWIDTH, PIECEHEIGHT);
        } else {
            contextRef.current.drawImage(image, figureX, figureY, PIECEWIDTH, PIECEHEIGHT, piece.draggingX - MARGIN_LEFT, piece.draggingY - MARGIN_TOP, PIECEWIDTH, PIECEHEIGHT);
            contextRef.current.shadowColor = "#d53";
            contextRef.current.shadowBlur = 20;
            contextRef.current.lineJoin = "bevel";
            contextRef.current.lineWidth = 5;
    ...
    const movePiece = (x, y) => {
        if (selectedPiece !== null && selectedPiece !== undefined) {
            ...
            if (grid !== null && selectedPiece !== null && selectedPiece.selected && !(grid.x === selectedPiece.x && grid.y === selectedPiece.y)) {
    ...
    const onMouseDown = ({ nativeEvent }) => {
        ...
        if (isWhiteTurn) {
            ...
            selectedPiece = getPiece(x, y, blackPieces);
        }
        if (selectedPiece !== null && selectedPiece !== undefined && ((isWhiteTurn && selectedPiece.color === WHITEPIECE) || (!isWhiteTurn && selectedPiece.color === BLACKPIECE))) {
    ...
    const onMouseUp = ({ nativeEvent }) => {
        ...
        if (selectedPiece !== null && selectedPiece !== undefined) {
            ...
            const grid = getGrid(x, y);
            if (grid !== null && selectedPiece !== null && !(grid.x === selectedPiece.x && grid.y === selectedPiece.y)) {
                if (selectedPiece.validMoveGrids.find(v => v.x === grid.x && v.y === grid.y)) {
                    ...
                    if (isWhiteTurn) {
                        ...
                        if (captureCheck(selectedPiece, whitePieces)) {
                            setWhitePieces([...whitePieces]);
                        }
                        setBlackPieces([...blackPieces]);
                    }
